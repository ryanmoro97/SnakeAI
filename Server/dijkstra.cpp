...
void dijkstra(const WDigraph& graph, int startVertex, unordered_map<int, PII>& tree) {
  BinaryHeap<int, PII> heap;

  tree.clear();

  // the PII key in the heap has the distance first and the vertex second, so
  // the min item in the heap according to < will have minimum distance

  heap.insert(startVertex, PII(0, -1));

  while (heap.size() > 0) {
    int v = heap.min().first;
    PII prev = heap.min().second;
